...
2026-09-10 更新：以下表格保留首版核心类别，不是当前所有测试文件清单。完整执行方式见[开发说明](../DEVELOPMENT.md)，当前 43 个文件的逐批清单与结果见[本次验收](../validation/INTEGRATION_2026-09-10.md)。新增 `test_model_paths_integration.py` 用相同模型和空白帧，对照英文目录与中文含空格目录，覆盖三个可选后端并验证子进程释放；不要用这类路径回归推断真人准确度。
