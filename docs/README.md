...
当前应用为 **0.17.0 顺畅流程版**：在 0.16 银发增量之上减少重复确认与重复校准、让普通提示不再打断，并新增引导计时练习与手动记次。使用和继续开发先读下面的入口文档；带版本号的旧文档用于追溯。
...
历史测试输出中的 109、1089、1102、1450 等数量属于各自阶段；0.15 / 0.16 包内验收也属于同学当时的运行。当前电脑最新执行的结果只看[0.17.0 本机验收](validation/SMOOTH_FLOW_V0_17_2026-09-12.md)，0.16 的接入结果见[9 月 12 日整合验收](validation/INTEGRATION_2026-09-12.md)，不能把各次结果相加。
