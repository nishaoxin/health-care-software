...
Windows 本地桌面 Demo，用普通摄像头或本地录像进行动作观察、评估记录和已确认计划的训练提示。当前 **0.17.0 顺畅流程版** 把重点放在顺利完成一次锻炼：摆好位置自动进入下一步、条件一致时沿用上次起点、普通看不清只给一条安静提示、开始前可取消倒计时、完成后就地看小结和下一步；自动测量撑不住时可改用**引导计时练习**按提示完成并手动记次，记录明确区分自动观察、近似观察、本人自报和未取得。保留 0.16 的银发增量、双摄逐项有效性与结果解读。未新增模型，不宣称全屋监护或正式远程救援。
