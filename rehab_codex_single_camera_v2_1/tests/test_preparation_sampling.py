...
@pytest.mark.parametrize('change', ['primary_identity', 'secondary_identity', 'size', 'context', 'stop', 'cancel'])
def test_sampling_cancels_instead_of_relabelling_changed_context(sampling, change):
    ...
    if change in ('stop', 'cancel'):
        ...
        if change == 'primary_identity':
            pose.people[0].track_key = 'changed-person'
        elif change == 'secondary_identity':
        ...
        if change in ('primary_identity', 'secondary_identity'):
...
def test_missing_geometry_can_recover_but_failed_window_never_samples_old_data(sampling):
    ...
    assert messages[-1]['kind'] == 'preparation' and '重试' in messages[-1]['text']
