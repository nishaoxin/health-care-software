...
阅读顺序：`SOFTWARE_SPEC.md` → `AGENTS.md` → `CHANGELOG_V2_1.md` → `START_WITH_CODEX.md`。v1迁移历史仍在`MIGRATION_V1_TO_V2.md`。
不要同时把旧v1作为首周并行指令。旧RTSP/雷达需求已隔离在`docs/FUTURE_EXTENSIONS.md`。
...
`docs/RESOURCES_AND_UPGRADES.md`是可选阅读，不是安装清单。本包不含研究权重、公开数据集、训练脚本或新成品业务逻辑。参考Python、配置与依赖沿用v2.0；本次主要更新Markdown文档，新增应用要求仍由Codex实现。
