...
class ExerciseCard(QFrame):
    ...
    def __init__(self, exercise_id, parent=None):
        ...
        box.setSpacing(10)
        cover = QFrame()
        cover.setObjectName('courseCover')
        cover.setFixedHeight(80)
        coverbox = QVBoxLayout(cover)
        coverbox.setContentsMargins(12, 10, 12, 10)
        line = QHBoxLayout()
        category = QLabel(JOINT_LABELS[info['joint']])
        category.setObjectName('coverCategory')
        line.addWidget(category)
        line.addStretch()
        view = QLabel(info['view_label'])
        view.setObjectName('coverMeta')
        line.addWidget(view)
        coverbox.addLayout(line)
        pending = QLabel('动作提示图待补充')
        pending.setObjectName('coverMeta')
        coverbox.addWidget(pending)
        box.addWidget(cover)
        top = QHBoxLayout()
        text = QVBoxLayout()
        text.setSpacing(4)
        ...
        box.addLayout(top)
        movement = QLabel(info['move'])
        movement.setObjectName('muted')
        movement.setWordWrap(True)
        box.addWidget(movement, 1)
        footer = QHBoxLayout()
        ...
        footer.addStretch()
        self.open_button = QPushButton('查看动作')
...
class ExerciseCatalog(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        herobox = QHBoxLayout(hero)
        herobox.setContentsMargins(24, 18, 24, 18)
        story = QVBoxLayout()
        eyebrow = QLabel('ASSESS  /  MOVE  /  RECORD')
        eyebrow.setObjectName('heroEyebrow')
        story.addWidget(eyebrow)
        hero_title = QLabel('从了解身体，开始今天的练习')
        hero_title.setObjectName('heroTitle')
        hero_title.setWordWrap(True)
        story.addWidget(hero_title)
        note = QLabel('按需选择动作 → 完成评估 → 确认计划 → 开始训练')
        note.setObjectName('heroDescription')
        note.setWordWrap(True)
        story.addWidget(note)
        herobox.addLayout(story, 1)
        links = QVBoxLayout()
        self.checklist = QPushButton('本轮评估清单')
        self.checklist.setObjectName('heroAction')
        self.checklist.clicked.connect(self.checklist_requested.emit)
        links.addWidget(self.checklist)
        self.body_button = QPushButton('查看评估结果')
        self.body_button.setObjectName('heroSecondary')
        self.body_button.clicked.connect(self.body_requested.emit)
        links.addWidget(self.body_button)
        herobox.addLayout(links)
        layout.addWidget(hero)
        heading = QHBoxLayout()
        title = QLabel('按部位选择动作')
        title.setObjectName('sectionTitle')
        ...
        self.search.setPlaceholderText('搜索动作或关节，例如：抬臂、腕、食指 PIP')
        ...
        note = QLabel('只选择已确认适合自己的动作，无需做完全部项目。所有角度均为二维观察值。')
