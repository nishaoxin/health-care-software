...
class ExerciseCard(QFrame):
    ...
    def __init__(self, exercise_id, parent=None):
        ...
        box.setSpacing(10)
        top = QHBoxLayout()
        top.addWidget(JointGlyph(info['joint']))
        text = QVBoxLayout()
        text.setSpacing(4)
        ...
        subtitle = QLabel(JOINT_LABELS[info['joint']]+'  ·  '+info['view_label'])
        ...
        footer = QHBoxLayout()
        tag = QLabel('实验性二维观察' if info['experimental'] else '二维动作观察')
...
class ExerciseCatalog(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.search.setPlaceholderText('搜索动作、手指或关节，如：食指 PIP')
        self.search.setClearButtonEnabled(True)
        self.search.setAccessibleName('搜索评估动作')
        self.search.setMaximumWidth(370)
        ...
        self.empty.setMinimumHeight(150)
        layout.addWidget(self.empty, 1)
        note = QLabel('按需要选择项目，左右侧分别记录。角度为二维观察值，不等同于临床关节活动度。')
    ...
    def _filter(self):
        ...
        for eid in ordered_ids:
            ...
            if (self.joint == 'all' or info['joint'] == self.joint) and all(
                    word in info['search_terms'].casefold() for word in words):
